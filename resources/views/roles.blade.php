...
<div class="d-sm-flex align-items-center justify-content-between pt-1 pb-1">
  <h1 class="h3 mb-0 text-body-secondary">Roles</h1>
</div>
<!-- fin Page Heading -->
...
<div class="row">
  ...
  <div class="col-lg-5 pe-0">
    ...
    <div class="card mb-3 rounded-0" style="box-shadow: 0px 3px 10px 2px #aaa;">
      ...
      <form id="formNuevoEditar" action="/admin/roles" method='POST'>    
        @csrf 
        <span id="metodo"></span>
        ...
        <div class="card-body">
          ...
          <div class="form-group" style="margin-top: -2%;">
            ...
            <div class="form-floating mb-0">
                <input type="text" class="form-control" id="nombreRol" name="nombreRol" value="{{ old('nombreRol') }}" placeholder="Nombre Rol" autocomplete="off">
                ...
                @foreach($roles as $rol)
                  <tr style="font-size: 14px;">
                    ...
                    <td class="text-center p-0" style="width: 17%;">
                      ...
                      <button type="button" class="btn btn-outline-danger" onclick=" $('#eliminar').attr('action','/admin/roles/{{ $rol->id }}');" data-bs-toggle="modal" data-bs-target="#modalEliminar">
...
<script>
  function editar(tr){
      ...
      $('#formNuevoEditar').attr('action','/admin/roles/'+tds.eq(0).text());
      $("#metodo > input").remove();
      $("#metodo").append('{{method_field("PATCH")}}');   
      $("#tituloCrearModificar").text('Modificar');
      
      $("#id").val(tds.eq(0).text());
      $("#nombreRol").val(tds.eq(1).text());
  }
  function cancelar(){
      $('#formNuevoEditar').attr('action','/admin/roles');
      $("#metodo > input").remove();
